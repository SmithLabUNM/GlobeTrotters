...
list.current = list.files(path = "./Data/Current", #change to appropriate path
                          ...
                          full.names = TRUE,
                          recursive = TRUE)
current.names <- gsub("./Data/Current/",
                      "",
                      list.current)
...
present.nat.names <- gsub("./Data/Present_natural/",
                          "",
                          list.present.nat)
...
mm.df <- mm.df %>%
  ...
  # Other marine/aquatic mammals:
  # Otters
  filter(genus != "Lontra") %>%
  filter(genus != "Lutra") %>%
  #polar bears
  filter(binomial != "Ursus maritimus")
...
nrow(mm.df[mm.df$diet.src == "species.diet",]) #3729
...
length(unique(mm.df$binomial)) #4380
nrow(df.contTaxaSums) #4380
...
#phyls <- read.nexus(file = "./Data/Complete_phylogeny.nex")
#1000 trees
...
df.origin.gen.foss <- left_join(df.origin.gen, foss.age,
                                by = "binomial")
...
df$dispersal.age.d[df$dispersal.age.d == -999] <- NA
...
length(unique(df$binomial[!is.na(df$dispersal.age.d)])) #106
# generation length from Pacifici
length(unique(df$binomial[!is.na(df$GenerationLength_d)])) #3845

## fossil ages
# pbdb
length(unique(df$genus[!is.na(df$foss.age)])) #482
length(unique(df$binomial[df$foss.age > 0])) #851
# faurby
length(unique(df$genus[!is.na(df$age.median)])) #1019
length(unique(df$binomial[!is.na(df$age.median)])) #3879
...
length(unique(df$family)) #135
length(unique(df$genus)) #1069
length(unique(df$binomial)) #4380
...
## COUNT OF BODY MASS
nrow(df[!is.na(df$log.mass),]) #3311
...
nrow(df[df$continent.North.America == TRUE,]) #804
nrow(df[df$continent.North.America == TRUE & is.na(df$log.mass),]) #92 (out of 804; 11.4%)

nrow(df[df$continent.South.America == TRUE,]) #1198
nrow(df[df$continent.South.America == TRUE & is.na(df$log.mass),]) #277 (out of 1198; 23.1%)

nrow(df[df$continent.Eurasia == TRUE,]) #1163
nrow(df[df$continent.Eurasia == TRUE & is.na(df$log.mass),]) #325 (out of 1163; 28%)
...
##### FOSSIL AGES ----
## PBDB COUNTS
length(unique(df$order[df$foss.age >= 0]))  #25
length(unique(df$family[df$foss.age >= 0])) #109
length(unique(df$genus[df$foss.age >= 0])) #483
length(unique(df$binomial[df$foss.age >= 0])) #851
nrow(df[is.na(df$foss.age),]) #3530
...
length(unique(df$genus[df$age.median >= 0])) #1020
length(unique(df$binomial[df$age.median >= 0])) #3880
nrow(df[is.na(df$age.median),]) #501
...
#p-value: 1.726e-10; Adjusted R-squared: 0.04747
#basically 0, no explanitory power
...
#p-value: 0.005137; Adjusted R-squared: 0.002288
...
table(df$log.size.bin[is.na(df$dispersal.age.d)])
# 0    1    2    3    4    5    6    7 
# 385 1312  696  389  229  148   44    2 

nrow(df[!is.na(df$dispersal.age.d),]) #106
nrow(df[!is.na(df$GenerationLength_d),]) #3845

##### CONTINENT OF ORIGIN ------
## FAMILY ORIGIN

length(unique(df$family[df$family.origin != ""])) #128
nrow(df[df$family.origin != "",]) #3566
...
## counts
length(unique(df$binomial[df$continent.North.America == TRUE])) #804
length(unique(df$binomial[df$continent.South.America == TRUE])) #1198
length(unique(df$binomial[df$continent.Eurasia == TRUE])) #1163
...
# diet.invertivore.tot   1421   549
# diet.carnivore.tot      281    48
# diet.browser.tot       1240   380
# diet.grazer.tot         495   119
# diet.piscivore.tot       24     1
...
#missing age
nrow(dd[is.na(dd$foss.age),]) #426
nrow(dd[is.na(dd$age.median),]) #36
...
df.dispersal %>% 
  drop_na() %>%
  summarise(n = n()) #76 records that have everything

df.dispersal %>%
  dplyr::select(-foss.avg.age) %>%
  drop_na() %>%
  summarise(n = n()) #100 for phylo age
...
tot <- length(unique(df$binomial)) #4380
n.one <- length(unique(df$binomial[df$n.cont == 1])) #4116
per.one <- n.one/tot #93.97%
n.two <- length(unique(df$binomial[df$n.cont == 2])) #258
per.two <- n.two/tot #5.89%
n.more <- length(unique(df$binomial[df$n.cont == "3+"])) #6
per.more <- n.more/tot #0.14%

length(unique(df$binomial[df$n.cont > 1])) #264
#258/264 = 0.9772727 are on 2 cont.
...
#of the ones on 2+ continents, what are they?
sort(table(df$order[df$n.cont > 1])) #143 are bats (143/264 = 54%)
...
nrow(df[df$continent.Africa == TRUE,]) #1150
nrow(df[df$continent.Africa == TRUE,])/tot #26.3
...
nrow(df[df$n.cont != 1 & 
        ...
        df$continent.South.America == TRUE,]) #162
nrow(df[df$n.cont != 1 & 
        df$continent.North.America == TRUE &
...
nrow(df[df$n.cont != 1 & 
        df$continent.North.America == TRUE &
        df$continent.Eurasia == TRUE,]) #19
nrow(df[df$n.cont != 1 & 
        df$continent.North.America == TRUE &
        df$continent.Eurasia == TRUE,])/tot #.4%
nrow(df[df$n.cont != 1 & 
        df$continent.North.America == TRUE &
...
# only 1, the fennec fox (Vulpes zerda), on 2 continents; the rest on 1
...
#other one is cave bear, Ursus spelaeus; both on 1
...
# 1 on two (Mustela putorius); the rest on 1
...
nrow(df[df$family.origin == "Eurasia" & 
          ...
          df$diet.breadth == 1,] %>%
         drop_na(order))
# 16 things similar (including Cervus elephas); all on 1
...
#2 things on 2: Smilodon fatalis, Panthera onca
...
## how many species are shared?
nrow(df[df$continent.Africa == TRUE & df$continent.Australia == TRUE,]) #1
nrow(df[df$continent.Africa == TRUE & df$continent.Eurasia == TRUE,]) #85
nrow(df[df$continent.Africa == TRUE & df$continent.North.America == TRUE,]) #5
nrow(df[df$continent.Africa == TRUE & df$continent.South.America == TRUE,]) #1
nrow(df[df$continent.Australia == TRUE & df$continent.Eurasia == TRUE,]) #5
nrow(df[df$continent.Australia == TRUE & df$continent.North.America == TRUE,]) #0
nrow(df[df$continent.Australia == TRUE & df$continent.South.America == TRUE,]) #0
nrow(df[df$continent.Eurasia == TRUE & df$continent.North.America == TRUE,]) #19
nrow(df[df$continent.Eurasia == TRUE & df$continent.South.America == TRUE,]) #1
nrow(df[df$continent.South.America == TRUE & df$continent.North.America == TRUE,]) #162
...
length(df$family.origin[df$family.origin != ""]) #3566
...
nrow(df[df$continent.North.America == TRUE
        & df$continent.South.America == TRUE
        & df$n.cont == 2
        & df$family.origin != "",]) #144
...
length(df.dispersal$dispersal.foss[!is.na(df.dispersal$dispersal.foss)]) #77

ggplot() +
  geom_density(aes(df.dispersal$dispersal.phylo[!is.na(df.dispersal$dispersal.phylo)]))
length(df.dispersal$dispersal.phylo[!is.na(df.dispersal$dispersal.phylo)]) #100

nrow(df.dispersal[is.na(df.dispersal$dispersal.foss),]) #3037
nrow(df.dispersal[is.na(df.dispersal$dispersal.phylo),]) #3014
length(unique(df.dispersal$family[is.na(df.dispersal$dispersal.foss)])) #123
length(unique(df.dispersal$family[is.na(df.dispersal$dispersal.phylo)])) #122
...
nrow(df.dispersal[!is.na(df.dispersal$dispersal.foss) &
                    df.dispersal$order == "Rodentia",]) #15; only have 15 rodents out of lots
nrow(df.dispersal[!is.na(df.dispersal$dispersal.foss) &
...
length(df.dispersal$binomial[!is.na(df.dispersal$dispersal.foss)]) #77

length(df.dispersal$binomial[df.dispersal$dispersal.foss >= Africa & !is.na(df.dispersal$dispersal.foss)]) 
length(df.dispersal$binomial[df.dispersal$dispersal.foss >= Australia & !is.na(df.dispersal$dispersal.foss)]) 
length(df.dispersal$binomial[df.dispersal$dispersal.foss >= Eurasia & !is.na(df.dispersal$dispersal.foss)]) 
length(df.dispersal$binomial[df.dispersal$dispersal.foss >= North.America & !is.na(df.dispersal$dispersal.foss)]) 
length(df.dispersal$binomial[df.dispersal$dispersal.foss >= South.America & !is.na(df.dispersal$dispersal.foss)]) 

length(df.dispersal$binomial[!is.na(df.dispersal$dispersal.phylo)]) #100

length(df.dispersal$binomial[df.dispersal$dispersal.phylo >= Africa & !is.na(df.dispersal$dispersal.phylo)]) 
length(df.dispersal$binomial[df.dispersal$dispersal.phylo >= Australia & !is.na(df.dispersal$dispersal.phylo)]) 
length(df.dispersal$binomial[df.dispersal$dispersal.phylo >= Eurasia & !is.na(df.dispersal$dispersal.phylo)]) 
length(df.dispersal$binomial[df.dispersal$dispersal.phylo >= North.America & !is.na(df.dispersal$dispersal.phylo)]) 
length(df.dispersal$binomial[df.dispersal$dispersal.phylo >= South.America & !is.na(df.dispersal$dispersal.phylo)]) 
...
p.disp.age <- ggplot(df.dispersal[!is.na(df.dispersal$dispersal.phylo),]) +
    ...
    geom_hline(yintercept = (log10(Australia)),
               col = "#afa298", size = 1) + 
    geom_hline(yintercept = (log10(EA_AF_dist)),
               col = "#afa298", size = 1)
...
df.time[76,] #Peromyscus leucopus; still has a large geographic range!; in NA
...
which.min(df.dispersal$dispersal.phylo) #2595
df.dispersal[2595,] #Sciurus carolinensis; also large geographic range and suprisingly similar to Peromyscus leucopus; in NA

which.max(df.time$dispersal.distance) #71
df.time[71,] #Panthera leo
which.max(df.time$dispersal.10) #71
which.max(df.time$dispersal.100) #71
which.max(df.time$dispersal.1000) #71
which.max(df.time$dispersal.10000) #71
which.max(df.time$dispersal.100000) #71
which.max(df.dispersal$dispersal.phylo) #2058
df.dispersal[2058,] #Panthera tigris; large geogrpahic range, in EA, not a globe trotter
...
which.max(df.dispersal$dispersal.foss) #2055
which.max(df.dispersal$dispersal.phylo) #2058
df.dispersal$binomial[2055]
df.dispersal$binomial[2058]
...
sort(unique(df.dispersal$qtr.bin.dist.foss)) #up to (11.5,11.8]
sort(unique(df.dispersal$qtr.bin.dist.phylo)) #up to (11.2,11.5]
...
df.dispersal[df.dispersal$qtr.bin.dist == "(6,6.25]",] #panthera leo 
...
insect.stat <- df %>%
  ...
  dplyr::summarise(count = n())
sum(insect.stat$count) #1970 total
...
sum(homies.family$homies.N) #4116 sp
...
p.div.2 <- ggplot(df[df$n.cont == "2",]) + #do histogram; .25 log 
    ...
    scale_x_continuous(name = expression(Family~Diversity),
                       ...
                       labels = c(expression(10^-1), expression(10^0), expression(10^1), 
                                  expression(10^2), expression(10^3)))
#sample size
length(rangers.family$rangers.N) #53 fam
sum(rangers.family$rangers.N) #258 sp
...
p.div.2.nonvol <- ggplot(df[df$n.cont == "2" & df$habitat.mode != "volant",]) + #do histogram; .25 log 
    ...
    scale_x_continuous(name = expression(Family~Diversity),
                       ...
                       labels = c(expression(10^-1), expression(10^0), expression(10^1), 
                                  expression(10^2), expression(10^3)))
#sample size
length(rangers.family.nonvol$rangers.N) #40 fam
sum(rangers.family.nonvol$rangers.N) #116 sp
...
p.bs.2 <- ggplot() + #do histogram; .25 log 
    ...
    scale_x_continuous(name = expression(log[10]~Body~Mass~(g)),
                       ...
                       labels = c(expression(10^0), expression(10^2), 
                                  expression(10^4), expression(10^6)))
#sample size
length(df$log.mass[df$n.cont == "2" & !is.na(df$log.mass)]) #241
...
p.bs.2.nonvol <- ggplot() + #do histogram; .25 log 
    ...
    scale_x_continuous(name = expression(log[10]~Body~Mass~(g)),
                       ...
                       labels = c(expression(10^0), expression(10^2), 
                                  expression(10^4), expression(10^6)))
#sample size
length(df$log.mass[df$n.cont == "2" & !is.na(df$log.mass) & df$habitat.mode != "volant"]) #112
...
p.bs.1 <- ggplot() + #do histogram; .25 log 
    ...
    scale_x_continuous(name = expression(log[10]~Body~Mass~(g)),
                       ...
                       labels = c(expression(10^0), expression(10^2), 
                                  expression(10^4), expression(10^6)))
#sample size
length(df$log.mass[df$n.cont == "1" & !is.na(df$log.mass)]) #3064
...
## diet breadth 2
table(df$n.cont[df$diet.breadth == 2 & df$diet.browser.tot == TRUE & df$diet.grazer.tot == TRUE])
table(df$n.cont[df$diet.breadth == 2 & df$diet.browser.tot == TRUE & df$diet.invertivore.tot == TRUE])
table(df$n.cont[df$diet.breadth == 2 & df$diet.browser.tot == TRUE & df$diet.frugivore.tot == TRUE])
table(df$n.cont[df$diet.breadth == 2 & df$diet.grazer.tot == TRUE & df$diet.carnivore.tot == TRUE])
table(df$n.cont[df$diet.breadth == 2 & df$diet.grazer.tot == TRUE & df$diet.invertivore.tot == TRUE])
table(df$n.cont[df$diet.breadth == 2 & df$diet.grazer.tot == TRUE & df$diet.frugivore.tot == TRUE])
table(df$n.cont[df$diet.breadth == 2 & df$diet.carnivore.tot == TRUE & df$diet.invertivore.tot == TRUE])
table(df$n.cont[df$diet.breadth == 2 & df$diet.carnivore.tot == TRUE & df$diet.frugivore.tot == TRUE])
table(df$n.cont[df$diet.breadth == 2 & df$diet.carnivore.tot == TRUE & df$diet.piscivore.tot == TRUE])
table(df$n.cont[df$diet.breadth == 2 & df$diet.invertivore.tot == TRUE & df$diet.frugivore.tot == TRUE])
table(df$n.cont[df$diet.breadth == 2 & df$diet.invertivore.tot == TRUE & df$diet.piscivore.tot == TRUE])
...
##sample size
length(df$diet.breadth[df$n.cont == "1"]) #4116
...
p.diet.brd.2 <-  ggplot() +  
    ...
    scale_x_continuous(name = "Dietary breadth")
##sample size
length(df$diet.breadth[df$n.cont == "2"]) #258
...
##sample size
length(df$diet.breadth[df$n.cont == "2" & df$habitat.mode != "volant"]) #116
...
p.diet.2 <- ggplot() + 
    ...
    theme(axis.text.x = element_text(vjust = -1),
          axis.title.x = element_text(vjust = -1))
##sample size
sum(diettype_bargraph_full$V1[diettype_bargraph_full$numconts == "2"], na.rm = TRUE) #258
...
p.diet.2.nonvol <- ggplot() + 
    ...
    theme(axis.text.x = element_text(vjust = -1),
          axis.title.x = element_text(vjust = -1))
##sample size
sum(diettype_bargraph_full.nonvol$V1[diettype_bargraph_full.nonvol$numconts == "2"], na.rm = TRUE) #116
...
p.hab.1 <-  ggplot() +  
    ...
    scale_x_discrete(name = "Habitat Mode",
                     labels = c("terr" = "terrestrial",
                                ...
                                "volant" = "volant"))
##sample size
length(df.hab.trim$habitat.mode[df.hab.trim$n.cont == "1"]) #3513
...
p.hab.2 <-  ggplot() +  
    ...
    scale_x_discrete(name = "Habitat Mode",
                     labels = c("terr" = "terrestrial",
                                ...
                                "volant" = "volant"))
##sample size
length(df.hab.trim$habitat.mode[df.hab.trim$n.cont == "2"]) #256
...
p.hab.2.nonvol <-  ggplot() +  
    ...
    scale_x_discrete(name = "Habitat Mode",
                     labels = c("terr" = "terrestrial",
                                ...
                                "volant" = "volant"))
##sample size
length(df.hab.trim$habitat.mode[df.hab.trim$n.cont == "2" & df.hab.trim$habitat.mode != "volant"]) #114
...
model <- glm(wide.ranging ~ locomotion + log.mass + diet.type + fam.connectivity + fam.richness.y + age.median, 
...
logitor(wide.ranging ~ locomotion + log.mass + diet.type + fam.connectivity + fam.richness.y + age.median, 
        data = df.tree.trim)
...
df.cont$pan.gr.area[df.cont$pan.gr.area == -999] <- NA
df.cont$faurby.nat.range[df.cont$faurby.nat.range == 0] <- NA
df.cont$faurby.current.range[df.cont$faurby.current.range == 0] <- NA

df.pan$hmrg[df.pan$hmrg == -999] <- NA
df.pan <- df.pan[!is.na(df.pan$hmrg),]

df.pan <- subset(df.cont, !is.na(df.cont$pan.gr.area) & !is.na(df.cont$size))

df.faurby <- subset(df.cont, !is.na(df.cont$faurby.nat.range) 
                    & !is.na(df.cont$size) 
                    & df.cont$faurby.nat.range != 0 
                    & !is.na(df.cont$faurby.current.range)
                    & df.cont$faurby.current.range != 0) 

length(unique(df.pan$binomial)) #2942
length(unique(df.pan$binomial[df.pan$num.cont == 1])) #2706
length(unique(df.pan$binomial[df.pan$num.cont == 2])) #230
length(unique(df.pan$binomial[df.pan$num.cont == "3+"])) #6

length(unique(df.faurby$binomial)) # 2470
length(unique(df.faurby$binomial[df.faurby$num.cont == 1])) #2359
length(unique(df.faurby$binomial[df.faurby$num.cont == 2])) #106
length(unique(df.faurby$binomial[df.faurby$num.cont == "3+"])) #5

df.pan$ratio <- df.pan$pan.gr.area/df.pan$cont.tot.area
df.faurby$ratio.nat <- df.faurby$faurby.nat.range/df.faurby$cont.tot.area
df.faurby$ratio.cur <- df.faurby$faurby.current.range/df.faurby$cont.tot.area

df.pan$num.cont <- as.factor(df.pan$num.cont)
df.pan$num.cont <- relevel(df.pan$num.cont, 
                           ref = "1", 
                           levels = c("1", "2", "3+"))
df.faurby$num.cont <- as.factor(df.faurby$num.cont)
df.faurby$num.cont <- relevel(df.faurby$num.cont,
                              ref = "1",
                              levels = c("1", "2", "3+"))

##### HOMERANGE ----
#homerange to geographic range
summary(lm(log10(df.pan$pan.gr.area) ~ log10(df.pan$hmrg)))

p.hr <- ggplot(data = df.pan, 
               aes(x = logSize, y = log10(hmrg))) +
...
summary(lm(log10(df.pan$hmrg) ~ df.pan$logSize*df.pan$num.cont))
#first three = intercepts; next all slopes; cont3+ has a higher intercept which means that these ones start off larger
#interaction tells me if slopes are sig diff or not

#https://stats.stackexchange.com/questions/320727/interpreting-r-lm-output-with-factor-as-variable
#run to see if slopes sig diff from one another
#summaryAnova(lm(log10(df.pan$hmrg) ~ df.pan$logSize*df.pan$num.cont), 
#      type = 3)
emmeans(lm(log10(df.pan$hmrg) ~ df.pan$logSize*df.pan$num.cont), ~ num.cont)


#relevel to get intercept and se for each factor:
df.pan$num.cont <- relevel(df.pan$num.cont,
                              ref = "2")
summary(lm(log10(df.pan$hmrg) ~ df.pan$logSize*df.pan$num.cont))
...
##### PAN GEOG RANGE -----
p.geog.range.w.legend <- ggplot(data = df.pan, aes(x = logSize, y = log10(ratio))) +
  geom_point(alpha = 0.7, aes(col = num.cont)) +
  geom_smooth(aes(color = num.cont), method = "lm") +
  scale_color_manual(values = cont_bw) +
  #scale_fill_manual(values = cont_bw) +
...
p.geog.range <- ggplot(data = df.pan, aes(x = logSize, y = log10(ratio))) +
    geom_point(alpha = 0.7, aes(col = num.cont)) +
    geom_smooth(aes(color = num.cont), method = "lm") +
    scale_color_manual(values = cont_bw) +
    #scale_fill_manual(values = cont_bw) +
...
emmeans(lm(log10(df.pan$ratio) ~ df.pan$logSize*df.pan$num.cont), ~ num.cont)

df.pan$num.cont <- relevel(df.pan$num.cont,
                           ref = "1")
summary(lm(log10(df.pan$ratio) ~ df.pan$logSize*df.pan$num.cont))
...
##### FAURBY RANGES -----
#compare geographic ranges
ggplot(df.cont) +
    geom_smooth(aes(x = faurby.nat.range, y = faurby.current.range))
...
## CURRENT RANGE
emmeans(lm(log10(df.faurby$ratio.cur) ~ df.faurby$logSize*df.faurby$num.cont), ~ num.cont)

df.faurby$num.cont <- relevel(df.faurby$num.cont,
                              ref = "1")
summary(lm(log10(df.faurby$ratio.cur) ~ df.faurby$logSize*df.faurby$num.cont)) 
...
## NATURAL RANGE
emmeans(lm(log10(df.faurby$ratio.nat) ~ df.faurby$logSize*df.faurby$num.cont), ~ num.cont)

df.faurby$num.cont <- relevel(df.faurby$num.cont,
                              ref = "1")
summary(lm(log10(df.faurby$ratio.nat) ~ df.faurby$logSize*df.faurby$num.cont)) 
